...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat startLineX = (_direction == horizonal ? _circleRadius : (self.frame.size.width - _lineWidth) / 2);
    CGFloat startLineY = (_direction == horizonal ? (self.frame.size.height - _lineWidth) / 2 : _circleRadius);//起点
    
    CGFloat endLineX = (_direction == horizonal ? self.frame.size.width - _circleRadius : (self.frame.size.width - _lineWidth) / 2);
    CGFloat endLineY = (_direction == horizonal ? (self.frame.size.height - _lineWidth) / 2 : self.frame.size.height- _circleRadius);//终点
    ...
    CGFloat slidedLineX = (_direction == horizonal ? MAX(_circleRadius, (_ratioNum * self.frame.size.width - _circleRadius)) : startLineX);
    
    CGFloat slidedLineY = (_direction == horizonal ? startLineY : MAX(_circleRadius, (_ratioNum * self.frame.size.height - _circleRadius)));
    ...
    CGFloat circleX = (_direction == horizonal ? MAX(_circleRadius + penWidth, slidedLineX - penWidth ) : startLineX);
    CGFloat circleY = (_direction == horizonal ? startLineY : MAX(_circleRadius + penWidth, slidedLineY - penWidth));
...
- (void)updateTouchPoint:(NSSet*)touches {
    ...
    self.ratioNum = (_direction == horizonal ? touchPoint.x : touchPoint.y) / (_direction == horizonal ? self.frame.size.width : self.frame.size.height);
