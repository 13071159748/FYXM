...
#import "GYCoverController.h"

@interface GYCoverController ()<UIGestureRecognizerDelegate>
...
@implementation GYCoverController
                ...
                if (self.pendingController) {
                    ...
                    if (self.isLeft) {
                        if (self.isCoverPage) {
                            ...
                            self.pendingController.view.frame = CGRectMake(-ViewWidth + tempPoint.x, 0, ViewWidth, ViewHeight);
                            self.currentController.view.frame = CGRectMake(tempPoint.x>0?tempPoint.x:0, 0, ViewWidth, ViewHeight);
                        }
            ...
            if (self.openAnimate) { // 动画
                ...
                if (self.pendingController) {
                    ...
                    if (self.isLeft) {
                        ...
                        if ((self.currentController.view.frame.origin.x >= -1 && self.isCoverPage == YES) ||
                            (self.currentController.view.frame.origin.x >= -ViewWidth*0.15 && self.isCoverPage == NO)) {
    ...
    if (self.pendingController) {
        ...
        if (self.isLeft) { // 左边
            ...
            if (animated) {
                ...
                __weak GYCoverController *weakSelf = self;
 ...
 *  @return 上一个显示控制器
 */
- (UIViewController *)getAboveCntroller {
 ...
 *  @return 下一个显示控制器
 */
- (UIViewController *)getBelowCntroller {
...
#pragma mark - - -
- (void)udToControllerAbove:(BOOL)isAbove {
    self.pendingController = isAbove == false ? [self getBelowCntroller] : [self getAboveCntroller];
    [self udAnimation:isAbove];
}

/**
 *  手势结束
 */
- (void)udAnimation:(BOOL)isAbove
{
    if (self.pendingController) {
        
        if (isAbove) { //  向上
            _pendingController.view.frame = CGRectMake(0, -ViewHeight, ViewWidth, ViewHeight);
            [self addChildViewController:_pendingController];
            [self.view addSubview:_pendingController.view];
            
            __weak GYCoverController *weakSelf = self;
            
            [UIView animateWithDuration:AnimateDuration animations:^{
                weakSelf.currentController.view.frame = CGRectMake(0, ViewHeight, ViewWidth, ViewHeight);
                weakSelf.pendingController.view.frame = CGRectMake(0, 0, ViewWidth, ViewHeight);
            } completion:^(BOOL finished) {
                [weakSelf animateSuccess:finished];
            }];
        }else{ // 向下
            _pendingController.view.frame = CGRectMake(0, ViewHeight, ViewWidth, ViewHeight);
            [self addChildViewController:_pendingController];
