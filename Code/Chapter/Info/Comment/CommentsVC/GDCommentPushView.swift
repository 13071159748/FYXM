...
import UIKit
  ...
  */
//评论view
class GDCommentPushView: UIView,UITextViewDelegate {
    ...
    override init(frame: CGRect) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(GDCommentPushView.keyboardWillShow(_:)), name: NSNotification.Name.UIKeyboardWillShow, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(GDCommentPushView.keyboardWillHide(_:)), name: NSNotification.Name.UIKeyboardWillHide, object: nil)
        addTGR(self, action: #selector(GDCommentPushView.commentendEdit), view: self)
        createUI()
    }
...
//        textView.endEditing(true)
    ...
    func createUI() {
        ...
        addTGR(self, action: #selector(GDCommentPushView.backViewTouche), view: backView)
...
//        close.setImage(UIImage.init(named: "reader_commentEdit_close"), for: UIControlState())
        close.addTarget(self, action: #selector(GDCommentPushView.closeBtnClick(_:)), for: .touchUpInside)
        let closeimage = UIImage(named: "reader_commentEdit_close")?.withRenderingMode(.alwaysTemplate)
        close.setImage(closeimage, for: UIControlState())
        close.tintColor = UIColor.black

        close.imageEdgeInsets = UIEdgeInsetsMake(5, 5, 5, 5)

        backView.addSubview(close)
        
        let push = UIButton(frame: CGRect (x: screenWidth - 60, y: 0, width: 60, height: 44))
        push.setTitle("发布", for: UIControlState())
        push.setTitleColor(mainColor, for: UIControlState())
        backView.addSubview(push)
        push.addTarget(self, action: #selector(GDCommentPushView.pushBtnClick(_:)), for: .touchUpInside)
