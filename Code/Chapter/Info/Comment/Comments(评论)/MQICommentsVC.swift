...
class MQICommentsVC: MQIBaseViewController {
    ...
    fileprivate var allVC = [MQICommentsDetailVC]()
...
//        contentScrollView.delegate = self
        ...
        for i in 0..<3 {
            ...
            button.setTitleColor(UIColor.colorWithHexString("#000000"), for: .normal)
            button.setTitleColor(mainColor, for: .selected)
            button.tag = i + 100
            button.addTarget(self, action: #selector(MQICommentsVC.commentTypeBtnClick(_:)), for: .touchUpInside)
            button.titleLabel?.font = UIFont.systemFont(ofSize: 14)
            selectedView.addSubview(button)
            
            let height = contentScrollView.height
            let detailVC = MQICommentsDetailVC.create() as! MQICommentsDetailVC
    ...
    func addTopView() {
        ...
        bookCommendCount.isHidden = true
    ...
    func addBottomView() {
        ...
        bottomButton.setTitle("发布评论", for: .normal)
        bottomButton.backgroundColor = mainColor
        bottomButton.setTitleColor(UIColor.colorWithHexString("#ffffff"), for: .normal)
        contentView.addSubview(bottomButton)
        bottomButton.addTarget(self, action: #selector(MQICommentsVC.pushComnents), for: .touchUpInside)
    }
