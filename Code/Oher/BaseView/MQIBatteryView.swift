...
/// 电池宽推荐使用宽高
var GYBatterySize:CGSize = CGSize(width: 25, height: 10)

/// 电池量宽度 跟图片的比例
private var HJBatteryLevelViewW:CGFloat = 20
private var HJBatteryLevelViewScale = HJBatteryLevelViewW / GYBatterySize.width

import UIKit


class MQIBatteryView: UIImageView {
    ...
    convenience init() {
        ...
        self.init(frame: CGRect(x: 0, y: 0, width: GYBatterySize.width, height: GYBatterySize.height))
    }
    
    /// 初始化
    override init(frame: CGRect) {
        
        super.init(frame: CGRect(x: 0, y: 0, width: GYBatterySize.width, height: GYBatterySize.height))
    ...
    override func layoutSubviews() {
        ...
        let spaceW:CGFloat = 1 * (frame.width / GYBatterySize.width) * HJBatteryLevelViewScale
        let spaceH:CGFloat = 1 * (frame.height / GYBatterySize.height) * HJBatteryLevelViewScale
